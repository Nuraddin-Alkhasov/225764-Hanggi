    ...
    class RecipeBindingAdapter : AdapterBase
    {
        ...
        private void NewCommandExecuted(object parameter)
        {
            if (DialogVisible == Visibility.Hidden) 
            {
                Task T = Task.Run(() =>
                {
                    bool result = (new LocalDBAdapter("INSERT INTO Barcodes (Barcode, MR) VALUES ('No Barcode','No recipe')")).DB_Input();
                    UpdateBarcodeList();
                });
            }
                    ...
                    if (SelectedBarcode != null)
                    {
                        ...
                        else 
                        {
                            bool result = (new LocalDBAdapter("UPDATE Barcodes " +
                                                                "SET Barcode ='" + SelectedBarcodeBuffer.BC + "', MR = '" + SelectedBarcodeBuffer.MR_Name + "' " +
        ...
        public void UpdateBarcodeList()
        {
            ...
            Task T = Task.Run(async () =>
            {
                ...
                {
                    DataTable DT = (new LocalDBAdapter("SELECT Id, Barcode, MR " +
                                                       "FROM Barcodes;")).DB_Output();
                    BarcodeList.Clear();
                    if (DT.Rows.Count > 0)
                    {
                    ...
                    Recipes.Clear();
                    RecipeAdapter ra = (RecipeAdapter)ApplicationService.GetAdapter("RecipeAdapter");
                    Recipes = ra.Recipes;
